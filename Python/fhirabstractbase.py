            ...
            if is_list:
                if isinstance(val, list):
                    if len(val) > 0:
                        ...
                        js[jsname] = [v.as_json(cast) if hasattr(v, 'as_json') else (self._cast(v, typ) if cast else v) for v in val]
            else:
                if not isinstance(val, list):
                    found.add(of_many or jsname)
                    js[jsname] = val.as_json(cast) if hasattr(val, 'as_json') else (self._cast(val, typ) if cast else val)
